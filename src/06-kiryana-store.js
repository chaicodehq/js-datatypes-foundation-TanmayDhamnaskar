...
export function getAffordableItems(items, maxPrice) {
  return (!Array.isArray(items) || !Number.isFinite(maxPrice)) ? [] : items.filter(item => item.price <= maxPrice);
}
...
export function calculateTotal(items) {
  ...
  return items.reduce((cost, item) => cost + (item.price * item.qty), 0);
}
